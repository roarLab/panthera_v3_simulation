...
class EncoderOdom:
    def __init__(self, ticks_per_meter, base_width):
        ...
        #self.odom_pub = rospy.Publisher('/odom_linear_actuator', Odometry, queue_size=10)
        self.cur_x = 0
        self.cur_y = 0
        self.cur_theta = 0.0
...
class Node:
    def __init__(self):
        ...
        rospy.Subscriber("linear_actuator", Twist, self.cmd_vel_callback)
        #rospy.Subscriber("b", Twist, self.cmd_vel_callback)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if ('enc1' in vars()) and ('enc2' in vars()):
                rospy.logdebug(" Encoders %d %d" % (100, 100))
                self.encodm.update_publish(100, 100)
                self.updater.update()
            r_time.sleep()
